            ...
            async.waterfall([
                ...
                function(res, cb){
                    if (!res) {
                        ...
                        model.activationCodeModel.save({code:code, user: id}, function(err, newActivationCode){
                            if(!err)
                                cb(null, res);
                            else
                                cb(err);
...
module.exports = {
    ...
    createActivateNext: function (req,res,next) {
        createActivate(req,function (code,res) {
            req.activator = req.activator || {};
            if(code !== 200){
                _.extend(req.activator,{code:code,message:res, user: req.user});
            } else{
                _.extend(req.activator,{code: code, message:"Account Created Successfully.", user: res});
            }
            next();
        });
    },
    createActivationNext: function (req,res,next) {
        createActivationByCode(req,function(code,res) {
            req.activator = req.activator || {};
            if(code !== 200){
                _.extend(req.activator, {code:code, message:res, user: req.user});
            } else{
    ...
    completeActivateNext: function (req,res,next) {
        ...
        completeActivate(req,function (code,res) {
            req.activator = req.activator || {};
            if(code !== 200){
                _.extend(req.activator,{code:code,message:res, user: req.user});
            } else{
                _.extend(req.activator,{code: code, message:"Activation Successful.", user: res});
            }
            next();
        });
    },
    completeActivationNext: function (req,res,next) {
        //Cornix: added user to activator
        completeActivationByCode(req, function (code,res) {
            req.activator = req.activator || {};
            if(code !== 200){
                _.extend(req.activator,{code:code,message:res, user: req.user});
            } else{
